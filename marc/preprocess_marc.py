...
def convert_marc_to_jsonl(marc_root_dir, language_set, category_set):
    print(marc_root_dir)
        ...
        for language in language_set:
            for category in category_set:
                ...
                if split == 'train' or split == 'test':
                    file = os.path.join(marc_root_dir, split, language, category, f'{split}.tsv')
                else:
                    file = os.path.join(marc_root_dir, split, language, f'{split}.tsv')
                ...
                with open(file, encoding="utf8") as f:
                    for i, line in enumerate(f):
                        ...
                        data.append(example)
                if split == 'train' or split == 'test':
                    write_file = os.path.join(marc_root_dir, split, language, category, f'{split}.jsonl')
                else:
                    write_file = os.path.join(marc_root_dir, split, language, f'{split}.jsonl')

                # Delete first line before writing
                data = data[1:]
                ...
                print(language, split, category, len(data))

def convert_marc_ipt_to_jsonl(marc_root_dir, language_set):
    print(marc_root_dir)

    for split in ['train']:
        for language in language_set:
            data = []
            file = os.path.join(marc_root_dir, split, 'IPT', language, f'{split}.tsv')
            ...
            write_file = os.path.join(marc_root_dir, split, 'IPT', language, f'{split}.jsonl')
            ...
            print(language, 'IPT', split, len(data))
...
def main():
    ...
    marc_root_dir = 'marc'
    language_set = ['de', 'en', 'fr', 'jp', 'zh']
    category_set = ['apparel', 'home', 'musical_instruments', 'sports']

    print('\n\nConverting MARC data to jsonl format...')
    # Convert MARC data to jsonl format
    convert_marc_to_jsonl(marc_root_dir, language_set, category_set)
    # convert_marc_ipt_to_jsonl(marc_root_dir, language_set)

    os.makedirs(args.destdir + '/raw', exist_ok=True)
    os.makedirs(args.destdir + '/bin', exist_ok=True)

    # Extract input/labels
    for split, path in ('train', args.train), ('valid', args.valid):
        data = read_marc(path, langs=None)
        original_size = len(data)
        data = [sample for sample in data if sample['label'] != '-']
        # Drop first element
        data = data[1:]
        assert all(sample['label'] in ('0', '1') for sample in data)
        filtered_size = len(data)
        if filtered_size != original_size:
            print(f'Filtered {filtered_size}/{original_size} samples from {path}', file=sys.stderr)
        for name, field in ('input', 'text'), ('label', 'label'):
