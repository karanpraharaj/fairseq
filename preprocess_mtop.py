...
def convert_mtop_to_jsonl(mtop_root_dir, language_set):
    print(mtop_root_dir)
    ...
    for language in language_set:
        for split in ['train', 'eval', 'test']:
            ...
            with open(os.path.join(mtop_root_dir, language, f'{split}.txt'), encoding="utf8") as f:
            ...
            with open(os.path.join(mtop_root_dir, language, f'{split}.jsonl'), 'w') as f:
                for example in data:
                    json.dump(example, f)
                    f.write('\n')
...
def main():
    mtop_root_dir = '/Users/karanpraharaj/fairseq/mtop_partitioned'
    language_set = ['de', 'en', 'fr', 'es', 'hi', 'th']

    # Convert original MTOP txt files to JSONL
    # convert_mtop_to_jsonl(mtop_root_dir, language_set)
    
    parser = argparse.ArgumentParser(description='Tokenize and binarize MTOP data')
    parser.add_argument('--sentencepiece-model', required=True)
    parser.add_argument('--train', required=True, help='Training data in TSV format')
    parser.add_argument('--valid', required=True, help='Validation data in TSV format')
    ...
    # --train mtop_partitioned/train/de/0.jsonl \
    # --valid mtop_partitioned/eval/de/eval.jsonl \

    # Extract input/labels
    for split, path in ('train', args.train), ('valid', args.valid):
        data = read_mtop(path, langs=None)
        original_size = len(data)
        data = [sample for sample in data if sample['intent'] != '-']

        filtered_size = len(data)
        if filtered_size != original_size:
            print(f'Filtered {filtered_size}/{original_size} samples from {path}', file=sys.stderr)
        for name, field in ('input', 'text'), ('label', 'intent'):
